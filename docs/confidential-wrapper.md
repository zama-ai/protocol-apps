...
More information on the registry contract can be found in the [wrapper-registry.md](wrapper-registry.md) file.
...
On each chain the intended pauser is the chain's `ConfidentialWrapperPauser`, a roster contract administered by governance that forwards `pause()` to one or many wrappers; see [Pausing](pausing.md) and the [pauser runbook](deployment/deploy-wrapper-pauser-runbook.md).
