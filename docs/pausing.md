...
flowchart
    ...
    Protocol-DAO -- default admin (roster) --> Wrapper-Pauser
...
Every confidential wrapper exposes `pause()` to a single address, its `pauser()`, and `unpause()` to its owner. On each chain, governance points every wrapper's `pauser()` at one `ConfidentialWrapperPauser` through `setPauser`. That contract holds the roster (an OpenZeppelin `AccessControlEnumerable` role, readable with `pausers()`, changed by its default admin with `addPauser` / `removePauser`) and forwards `pause()` to one wrapper (`pause(address)`) or to many at once (`pause(address[])`, best effort: a wrapper that rejects the call, or an address the chain's `ConfidentialTokenWrappersRegistry` does not list as a valid wrapper, is reported with `WrapperPauseFailed` and the rest still stops; a wrapper that is already paused is reported with `WrapperAlreadyPaused`). It only ever calls addresses the registry lists, so a typo or a non-wrapper in a batch is reported, never executed. Its default admin is the chain's governance (`AccessControlDefaultAdminRules`: a single admin, moved only through a delayed two-step transfer), so adding or removing a roster member is a governance action, as is unpausing. A wrapper whose `pauser()` is the zero address cannot be paused by anyone.
