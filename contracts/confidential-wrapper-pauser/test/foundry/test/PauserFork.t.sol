...
/// @dev Registry enumeration path, mirrored from ConfidentialTokenWrappersRegistry (the pauser itself only needs
 ...
 * governance as default admin and the chain's registry as its allowlist, have governance point every registered
 * wrapper's `setPauser` at it, then let a single roster member halt everything and governance restore it. No FHE
 * is involved, so the suite needs only forge-std.
 *
...
contract PauserForkTest is Test {
    ...
    function setUp() public {
        ...
        // Every wrapper must be owned by the same governance address: it is the pauser's admin and the proposer
        ...
        // Migration step 1: anyone deploys, governance is the default admin, the roster is seeded.
        address[] memory roster = new address[](2);
        roster[0] = fb1;
        roster[1] = fb2;
        pauser = new ConfidentialWrapperPauser(
            ADMIN_DELAY,
            governance,
            IConfidentialTokenWrappersRegistry(address(registry)),
            roster
        );
        assertEq(pauser.owner(), governance, "pauser admin is not the wrappers' owner");
        assertEq(address(pauser.registry()), address(registry), "pauser registry is not the chain registry");
    }
    ...
    function test_ArmPauseAllAndRestore() public {
        ...
        // Governance is the admin, not a roster member.
        vm.prank(governance);
    ...
    /// @notice Only the live registry's valid wrappers are ever called: an unregistered address (a random EOA and
    /// the registry itself, a contract that is not a wrapper) is reported as WrapperNotRegistered without a call.
    function test_UnregisteredAddressesAreReportedNotCalled() public {
        _armAll();
        address[] memory batch = new address[](3);
        batch[0] = outsider;
        batch[1] = address(registry);
        batch[2] = wrappers[0];
        ...
        vm.expectEmit(true, false, false, true, address(pauser));
        emit IConfidentialWrapperPauser.WrapperPaused(wrappers[0]);
        vm.prank(fb1);
        pauser.pause(batch);
        assertTrue(ILiveWrapper(wrappers[0]).paused(), "registered wrapper not paused");

        vm.prank(fb1);
        vm.expectRevert(
            abi.encodeWithSelector(
                IConfidentialWrapperPauser.PauseFailed.selector,
                outsider,
                abi.encodeWithSelector(IConfidentialWrapperPauser.WrapperNotRegistered.selector, outsider)
            )
        );
        pauser.pause(outsider);
    }
    ...
    function test_GovernanceRotatesRoster() public {
        ...
        vm.prank(fb1);
        vm.expectRevert(
            abi.encodeWithSelector(IAccessControl.AccessControlUnauthorizedAccount.selector, fb1, bytes32(0))
        );
