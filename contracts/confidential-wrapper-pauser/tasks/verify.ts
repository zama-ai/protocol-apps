...
import { getRequiredAddressEnvVar, getRequiredAddressListEnvVar } from "./utils/loadVariables";
...
// variables the deploy script used (PAUSER_OWNER_ADDRESS, PAUSER_INITIAL_PAUSERS).
// Example usage:
...
task("task:verifyConfidentialWrapperPauser", "Verifies a deployed ConfidentialWrapperPauser on the enabled explorers")
  ...
  .setAction(async function ({ address }: { address: string }, hre) {
    ...
    await verifyOnEnabledExplorers(hre, address, [owner, initialPausers]);
  });
