...
/**
 * Resolves the registry to enumerate (and, at deploy time, to gate the pauser on): an explicit override first,
 * then the committed per-network config.
 */
export function resolveRegistry(hre: HardhatRuntimeEnvironment, override?: string): string {
  ...
  if (!config) {
    throw new Error(
      `No registry configured for network "${hre.network.name}"; pass --registry <address> (tasks) or set ` +
        `PAUSER_REGISTRY_ADDRESS (deploy).`,
    );
  }
