...
// Parse a non-negative integer number of seconds that fits a Solidity uint48 from an environment variable
export function getRequiredUint48EnvVar(name: string): bigint {
  const value = getRequiredEnvVar(name).trim();
  if (!/^\d+$/.test(value)) {
    throw new Error(`"${name}" env variable must be a non-negative integer (seconds): ${value}`);
  }
  const parsed = BigInt(value);
  if (parsed > 2n ** 48n - 1n) {
    throw new Error(`"${name}" env variable does not fit a uint48: ${value}`);
  }
  return parsed;
}
