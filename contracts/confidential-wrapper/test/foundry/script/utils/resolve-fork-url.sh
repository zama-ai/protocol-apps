...
#
# Prefers the process environment (CI injects CONFIDENTIAL_WRAPPER_UPGRADE_TEST_RPC_URL
...
URL="${CONFIDENTIAL_WRAPPER_UPGRADE_TEST_RPC_URL:-}"

if [ -z "${URL}" ] && [ -f ../../.env ]; then
  URL="$(. ../../.env && printf '%s' "${CONFIDENTIAL_WRAPPER_UPGRADE_TEST_RPC_URL:-}")"
fi

if [ -z "${URL}" ]; then
  echo "CONFIDENTIAL_WRAPPER_UPGRADE_TEST_RPC_URL is not set." >&2
