...
if (!existsSync(SOURCE)) {
  console.error(`❌ Template directory not found: ${SOURCE}`);
  process.exit(1);
}
...
console.log("📦 Adding trailofbits marketplace...");
execFileSync("claude", ["plugin", "marketplace", "add", "trailofbits/skills"], {
  stdio: "inherit",
});
...
console.log("🔌 Installing building-secure-contracts plugin...");
execFileSync(
  "claude",
  ["plugin", "install", "building-secure-contracts@trailofbits", "--scope", "project"],
  { stdio: "inherit" },
);
