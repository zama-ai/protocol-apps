...
task("task:allowHandle")
  .addParam(
    "handle",
    "The encrypted handle (bytes32) to allow",
    undefined,
    types.string,
  )
  .addParam(
    "account",
    "The account or contract address to allow the handle to",
    undefined,
    types.string,
  )
  .setAction(async function (
    ...
    console.log(
      `Allowing handle ${taskArguments.handle} to account ${taskArguments.account}...`,
    );
...
task("task:allowForSafeMultiSig")
  ...
  .setAction(async function (
    ...
    let helper;
    if (network.name === "mainnet") {
      helper = "0xd430F46fE522a32b12ce92C719f437fFce35e127";
    } else if (network.name === "testnet") {
      helper = "0xc51693587A5ec99FF131Ccd8aa6Fb424B17f5F61";
    } else {
      throw Error(`Unsupported network ${network.name}`);
    }
    const multiSigHelper = await ethers.getContractAt(
      "FHEVMMultiSigHelper",
      helper,
      proposer,
    );
